...
const CvDownloadModal = ({ isOpen, onClose }) => {
  ...
  return (
    ...
    >
      <div className="relative w-full max-w-md bg-white dark:bg-[#181C24] border border-slate-200 dark:border-slate-800 rounded-2xl shadow-2xl overflow-hidden p-6 sm:p-8 transition-all">
        ...
        <div className="flex flex-col items-center text-center">
          <div className="w-14 h-14 rounded-2xl bg-[#991B1B]/10 border border-[#991B1B]/30 flex items-center justify-center text-[#991B1B] dark:text-red-400 mb-4 shadow-inner">
            <FiLock className="w-7 h-7" />
          </div>
          <h3 id="modal-title" className="text-xl font-bold text-[#0F172A] dark:text-white">
            Acceso Protegido
          </h3>
        ...
        <form onSubmit={handleSubmit} className="mt-6 space-y-4">
          <div>
            ...
            <div className="relative">
              <input
                ...
                className="w-full px-4 py-3 pr-12 rounded-xl border border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-[#11141B] text-[#0F172A] dark:text-slate-100 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-[#991B1B]/40 focus:border-[#991B1B] transition-all text-sm"
                required
              />
              <button
                type="button"
                ...
                disabled={isLoading || isSuccess}
                className="absolute right-3 top-1/2 -translate-y-1/2 p-1.5 text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 rounded-lg transition-colors cursor-pointer"
          ...
          <div className="mt-6 flex flex-col sm:flex-row gap-3">
            <button
              ...
              onClick={handleClose}
              disabled={isLoading}
              className="w-full sm:w-1/2 px-4 py-3 rounded-xl border border-slate-300 dark:border-slate-700 text-slate-700 dark:text-slate-300 font-semibold text-sm hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors disabled:opacity-50 cursor-pointer"
            >
              Cancelar
            </button>
            <button
              ...
              className="w-full sm:w-1/2 px-4 py-3 rounded-xl bg-[#991B1B] hover:bg-[#7F1D1D] text-white font-bold text-sm shadow-md shadow-red-950/25 hover:shadow-lg transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 cursor-pointer"
            >
              {isLoading ? (
                <>
